...
def limpiar_fondo_hsv(img_color):
    """
    Nuevo filtro por color (HSV) para eliminar líneas azules del cuaderno.
    """
    hsv = cv2.cvtColor(img_color, cv2.COLOR_BGR2HSV)
    
    # Rango para detectar tonos azules/celestes
    lower_blue = np.array([80, 20, 100])
    upper_blue = np.array([140, 255, 255])
    
    mask = cv2.inRange(hsv, lower_blue, upper_blue)
    
    img_limpia = img_color.copy()
    img_limpia[mask > 0] = [255, 255, 255] # Reemplaza el azul por blanco
...
class Window(QtWidgets.QWidget):
    ...
    def procesar_todo(self):
        ...
        # Llamamos al nuevo filtro HSV
        self.procesedImage = limpiar_fondo_hsv(doc) 
    ...
    def analizar_lineas_y_caracteres(self, img_limpia_color):
        """
        Segmentación jerárquica: Párrafo -> Línea -> Caracteres
        """
        gray = cv2.cvtColor(img_limpia_color, cv2.COLOR_BGR2GRAY)
        _, bin_img = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        
        img_lineas_visual = img_limpia_color.copy()
        img_caracteres_visual = img_limpia_color.copy()

        # 1. Detectar Párrafos (Bloques grandes)
        ...
        # Ordenar párrafos de arriba hacia abajo
        ...
        for px, py, pw, ph in bboxes_p:
            if ph < 40 or pw < 40: continue # Ignorar manchas pequeñas
            
            # Recortar el área del párrafo en la imagen binarizada
            roi_parrafo_bin = bin_img[py:py+ph, px:px+pw]
            
            # 2. Detectar Líneas dentro del párrafo actual
            ...
            bboxes_l = [cv2.boundingRect(c) for c in contornos_l]
            bboxes_l = sorted(bboxes_l, key=lambda b: b[1])
            
            for lx, ly, lw, lh in bboxes_l:
                if lh < 10: continue # Ignorar líneas demasiado delgadas
                
                # Calcular coordenadas globales relativas a la imagen original
                global_x = px + lx
                global_y = py + ly
                
                # Dibujar rectángulo de la línea para el visualizador
                cv2.rectangle(img_lineas_visual, (global_x, global_y), (global_x + lw, global_y + lh), (255, 0, 0), 2)
                
                # Extraer recorte a color para mandarlo a EasyOCR
                recorte_color = img_limpia_color[global_y:global_y+lh, global_x:global_x+lw]
                lista_recortes.append(recorte_color)

                # 3. Detectar Caracteres dentro de la línea (para tu contador de OpenCV)
